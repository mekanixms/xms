...
abstract class CommonXml extends XmsOverload
{
    ...
    final public function attr()
    {
        ...
        if (!$this->attr_helper instanceof Closure)
            $this->attr_helper = function(&$el, $check, $attrName, $attrValue) {
        ...
        if (func_num_args() == 2) {
            if (function_exists($this->check))
                $this->each($this->attr_helper, $this->check, $attrName, $attrValue);
            else
                $this->each($this->attr_helper, FALSE, $attrName, $attrValue);
            return $this;
    ...
    final public function removeAttr()
    {
        if (func_num_args() > 0) {
            ...
            if (!$this->removeAttr_helper instanceof Closure)
                $this->removeAttr_helper = function(&$el, $check, $attrName) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el))) {
                        if ($el->nodeType == 1)
            ...
            if (function_exists($this->check))
                $this->each($this->removeAttr_helper, $this->check, $attrName);
            else
                $this->each($this->removeAttr_helper, FALSE, $attrName);
        }
        return $this;
    }
    ...
    final public function removeAttributes()
    {
        if (!$this->removeAttributes_helper instanceof Closure)
            $this->removeAttributes_helper = function(&$el, $check) {
                if (!$check || ($check && is_callable($check) && call_user_func($check, $el))) {
                    if ($el->nodeType == 1)
        ...
        if (function_exists($this->check))
            $this->each($this->removeAttributes_helper, $this->check);
        else
    ...
    final public function append()
    {
        if (func_num_args() == 1) {
            ...
            if (!$this->append_helper instanceof Closure)
                $this->append_helper = function(&$el, $check, $df, $cloneNode) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el)))
                        if ($cloneNode) {
            ...
            foreach ($nodes as $node)
                if (function_exists($this->check))
                    $this->each($this->append_helper, $this->check, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
                else
                    $this->each($this->append_helper, FALSE, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
        }
        return $this;
    }
    ...
    final public function prepend()
    {
        if (func_num_args() == 1) {
            ...
            if (!$this->prepend_helper instanceof Closure)
                $this->prepend_helper = function(&$el, $check, $df, $cloneNode) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el)))
                        if ($cloneNode) {
                            if (method_exists($df, "cloneNode"))
                                $newdf = $df->cloneNode(TRUE);
            ...
            foreach ($nodes as $node)
                if (function_exists($this->check))
                    $this->each($this->prepend_helper, $this->check, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
                else
                    $this->each($this->prepend_helper, FALSE, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
        }
        return $this;
    }
    ...
    final public function before()
    {
        if (func_num_args() == 1) {
            ...
            if (!$this->before_helper instanceof Closure)
                $this->before_helper = function(&$el, $check, $df, $cloneNode) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el)))
                        if ($cloneNode) {
                            if (method_exists($df, "cloneNode"))
                                $newdf = $df->cloneNode(TRUE);
            ...
            foreach ($nodes as $node)
                if (function_exists($this->check))
                    $this->each($this->before_helper, $this->check, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
                else
                    $this->each($this->before_helper, FALSE, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
        }
        return $this;
    }
    ...
    final public function after()
    {
        if (func_num_args() == 1) {
            ...
            if (!$this->after_helper instanceof Closure)
                $this->after_helper = function(&$el, $check, $df, $cloneNode) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el)))
                        if ($cloneNode) {
                            if (method_exists($df, "cloneNode"))
                                $newdf = $df->cloneNode(TRUE);
            ...
            foreach ($nodes as $node)
                if (function_exists($this->check))
                    $this->each($this->after_helper, $this->check, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
                else
                    $this->each($this->after_helper, FALSE, $node, (sizeof($this->results) > 1 ? TRUE : FALSE));
        }
        return $this;
    }
    ...
    {
        if (func_num_args() == 1) {
            if (!$this->replaceContent_helper_removeContent instanceof Closure)
                $this->replaceContent_helper_removeContent = function(&$el, $check) {
                    if (!$check || ($check && is_callable($check) && call_user_func($check, $el))) {
            ...
            if (function_exists($this->check))
                $this->each($this->replaceContent_helper_removeContent, $this->check);
            else
                $this->each($this->replaceContent_helper_removeContent, FALSE);

            $this->append(func_get_arg(0));
        }
        return $this;
    }
